...
public class CriterioTaxativo implements Criterio{
	...
	public List<Empresa> evaluar(List<String> listaPeriodos, List<Empresa> empresas) {
		...
		return empresas.stream().filter(empresa -> modificador.modificar(empresa,indicador,listaPeriodos).stream().allMatch(otroValor -> operador.aplicar(otroValor, valor))).collect(Collectors.toList());
